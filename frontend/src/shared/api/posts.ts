...
export const unifiedPublication = async (
	files: FileList,
	waterMark: boolean,
	chatIdList: string[],
	isInstant: boolean
...
export const receiving = async (
	...
	const { data } = await $autHost.get('posts/receiving', {
		params: { page, perpage, watched, channel, search }
	});
	return data;
};
...
export const receivingPostOrChangePage = async (
	id: number,
	where?: string,
	watched?: string,
	channel?: string
) => {
	const { data } = await $autHost.get(`posts/receiving-post-or-change-page/${id}`, {
		params: { where, watched, channel }
	});
	return data;
};

export const updatePosts = async (value: {
	id: number;
	channelIds?: number[];
	images?: IImageBlock[];
}) => {
...
export const receivingTags = async (params: {
	page: number;
	perPage: number;
	search?: string;
	sortBy?: string;
