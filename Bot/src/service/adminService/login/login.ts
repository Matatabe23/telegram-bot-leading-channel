...
export async function login(name: string, password: string) {
  if (!name || !password) throw new Error('Некорректные данные');

  const lowerName = name.toLowerCase();
  const admin = await administrators.findOne({ where: { name: lowerName } });
  if (!admin) throw new Error('Пользователь не найден');
  if (admin.dataValues.password !== password) throw new Error('Указан неверный пароль');
